...
public class KeyConverterToadlet extends WebInterfaceToadlet {
    ...
    public void handleMethodPOST(URI uri, HTTPRequest request, ToadletContext ctx)
        ...
        String newFilename = request.getPartAsStringFailsafe(Globals.PARAM_FILENAME,
    ...
    private void makeMainPage(ToadletContext ctx, List<String> errors, String origFileKey,
                              String newFilename, boolean getKeyOnly, boolean doIt)
            throws ToadletContextClosedException, IOException {
    ...
    private FreenetURI convert(List<String> errors, String origFileKey, String newFilename,
                               boolean getKeyOnly) {
        assert (errors != null) && (origFileKey != null) && (newFilename != null);
        ...
        try {
            Bucket      metadataBucket =
                metadata.toBucket(pluginContext.clientCore.tempBucketFactory);
            InsertBlock block          = new InsertBlock(metadataBucket, null,
                                             FreenetURI.EMPTY_CHK_URI);

            assert pluginContext.hlsc instanceof HighLevelSimpleClientImpl;

            HighLevelSimpleClientImpl hlsc          =
                (HighLevelSimpleClientImpl) pluginContext.hlsc;
            InsertContext             insertContext = hlsc.getInsertContext(true);

            insertContext.setCompatibilityMode(compatMode);
            newFileKey = hlsc.insert(block, getKeyOnly, newFilename, true,
                                     RequestStarter.INTERACTIVE_PRIORITY_CLASS, insertContext,
                                     cryptoKey);
        } catch (IOException e) {
            errors.add("IO error: " + e.getMessage());
    ...
    private HTMLNode createFormBox(String origFileKey, String newFilename, boolean getKeyOnly) {
        ...
        InfoboxNode box =
            pluginContext.pageMaker.getInfobox(
                "Convert a file key of any type to CHK with optional filename changing");
        ...
        form.addChild("br");
        form.addChild("input", new String[] { "type", "name", "size", "value" },
                      new String[] { "text",
                                     Globals.PARAM_URI, "70", origFileKey });
        form.addChild("br");
        form.addChild("#", "New filename:");
        form.addChild("br");
        form.addChild("input", new String[] { "type", "name", "size", "value" },
                      new String[] { "text",
                                     Globals.PARAM_FILENAME, "70", newFilename });
        form.addChild("br");

        if (getKeyOnly) {
            form.addChild("input", new String[] { "type", "name", "value", "checked" },
                          new String[] { "checkbox",
                                         PARAM_GETKEYONLY, "ok", "checked" });
        } else {
        ...
        form.addChild("br");
        form.addChild("br");
        form.addChild("input", new String[] { "type", "name", "value" }, new String[] { "submit",
                "debug", "Convert" });
    ...
    private HTMLNode createResultBox(FreenetURI newFileKey, boolean getKeyOnly) {
        ...
        InfoboxNode box =
            pluginContext.pageMaker.getInfobox("infobox-information",
                getKeyOnly
                ? "The file key has NOT been converted (only a new key was calculated)."
                : "The file key has been successfully converted.");
        HTMLNode content = box.content;
