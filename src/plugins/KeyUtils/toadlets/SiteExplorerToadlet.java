...
public class SiteExplorerToadlet extends WebInterfaceToadlet {
    ...
    public void handleMethodPOST(URI uri, HTTPRequest request, ToadletContext ctx)
        ...
        try {
            furi = URISanitizer.sanitizeURI(errors, key, false, URISanitizer.Options.NOMETASTRINGS,
                    URISanitizer.Options.SSKFORUSK);
            key = furi.toString(false, false);
        } catch (MalformedURLException e) {
            if (logDEBUG) {
                Logger.debug(this, "debug", e);
            }
    ...
    private void makeMainPage(ToadletContext ctx, List<String> errors, String key, boolean deep,
                              boolean ml)
            throws ToadletContextClosedException, IOException {
        PageNode page        = pluginContext.pageMaker.getPageNode(i18n("SiteExplorer.PageTitle"),
                                   ctx);
        ...
        HTMLNode uriBox = createUriBox(pluginContext,
                              ((furi == null) ? null : furi.toString(false, false)), deep, errors);
    ...
    private HTMLNode createUriBox(PluginContext pCtx, String uri, boolean deep,
                                  List<String> errors) {
        ...
        browseForm.addChild("input", new String[] { "type", "name", "value" },
                            new String[] { "hidden",
                Globals.PARAM_MFTYPE, Globals.MFTYPE_AUTO });
    ...
    private void makeManifestPage(ToadletContext ctx, List<String> errors, String key, boolean zip,
                                  boolean tar, boolean deep, boolean ml)
            throws ToadletContextClosedException, IOException {
        PageNode   page        =
            pluginContext.pageMaker.getPageNode(i18n("SiteExplorer.PageTitle"), ctx);
        ...
        try {
            ...
            if (zip) {
                ...
                metadata = KeyExplorerUtils.tarManifestGet(pluginContext.pluginRespirator, furi,
                        ".metadata");
            } else {
                metadata = KeyExplorerUtils.simpleManifestGet(pluginContext.pluginRespirator, furi);

                if (ml) {
                    metadata = KeyExplorerUtils.splitManifestGet(pluginContext.pluginRespirator,
                            metadata);
                }
            }
        } catch (MalformedURLException e) {
        ...
        if (errors.size() > 0) {
            ...
            contentNode.addChild(createUriBox(pluginContext,
                    ((furi == null) ? "" : furi.toString(false, false)), deep, errors));
            writeHTMLReply(ctx, 200, "OK", pageNode.generate());

            return;
        }

        contentNode.addChild(createUriBox(pluginContext, furi.toString(false, false), deep,
                errors));
        ...
        parseMetadataItem(contentTable, "", metadata, "", furi.toString(false, false), errors,
                          deep, 0, -1);
    ...
    private void parseMetadataItem(HTMLNode htmlTable, String name, Metadata md, String prefix,
            String furi, List<String> errors, boolean deep, int nestedLevel, int subLevel) {
        ...
        if (md.isSimpleManifest()) {
            ...
            if (defaultDoc != null) {
                parseMetadataItem(htmlTable, "/", defaultDoc, prefix + name, furi, errors, deep,
                                  nestedLevel, subLevel + 1);
            }

            for (Entry<String, Metadata> entry : docs.entrySet()) {
                parseMetadataItem(htmlTable, entry.getKey(), entry.getValue(), prefix + name + '/',
                                  furi, errors, deep, nestedLevel, subLevel + 1);
            }
        ...
        if (md.isMultiLevelMetadata()) {
            ...
            if (md.isSingleFileRedirect()) {
                htmlTableRow.addChild(makeCell(new HTMLNode("a", "href",
                        KeyUtilsPlugin.PLUGIN_URI + "/?key="
                        + md.getSingleTarget().toString(false,
                            false), md.getSingleTarget().toString(false, false))));
            } else {
            ...
            if (deep) {
                ...
                try {
                    if (md.isSingleFileRedirect()) {
                        ...
                        subMd = KeyExplorerUtils.simpleManifestGet(pluginContext.pluginRespirator,
                                mlUri);
                    } else {
                        subMd = KeyExplorerUtils.splitManifestGet(pluginContext.pluginRespirator,
                                md);
                    }

                    parseMetadataItem(htmlTable, "", subMd, prefix + name, furi, errors, deep,
                                      nestedLevel + 1, -1);
        ...
        if (md.isSimpleRedirect()) {
            ...
            if (deep && md.isNoMimeEnabled() && md.isSingleFileRedirect()) {
                ...
                if ((mlUri.getExtra()[2] & 0x02) != 0) {
                    ...
                    try {
                        Metadata subMd =
                            KeyExplorerUtils.simpleManifestGet(pluginContext.pluginRespirator,
                                mlUri);
                        ...
                        if (subMd.isMultiLevelMetadata()) {
                            ...
                            subMd =
                                KeyExplorerUtils.splitManifestGet(pluginContext.pluginRespirator,
                                    subMd);
                        }
        ...
        if (md.isArchiveMetadataRedirect()) {
            ...
            if (deep) {
                ...
                try {
                    ...
                    metadata = KeyExplorerUtils.tarManifestGet(pluginContext.pluginRespirator, u,
                            md.getArchiveInternalName());

                    // parse into
                    parseMetadataItem(htmlTable, "", metadata, prefix + name, furi, errors, deep,
                                      nestedLevel + 1, -1);
        ...
        if (md.isArchiveManifest()) {
            ...
            if (md.isSingleFileRedirect()) {
                ...
                htmlTableRow.addChild(makeCell(new HTMLNode("a", "href",
                        KeyUtilsPlugin.PLUGIN_URI + "/?mftype=" + md.getArchiveType().name()
                        + "manifest&key=" + containerTarget, containerTarget)));
            } else {
                htmlTableRow.addChild(makeEmptyCell());
            }
            ...
            if (deep) {
                ...
                if (md.getArchiveType() == ARCHIVE_TYPE.TAR) {
                    try {
                        if (md.isSplitfile()) {
                            subMd = KeyExplorerUtils.tarManifestGet(pluginContext.pluginRespirator,
                                    md, ".metadata");
                        } else {
                            subMd = KeyExplorerUtils.tarManifestGet(pluginContext.pluginRespirator,
                                    md.getSingleTarget(), ".metadata");
                        }
    ...
    private HTMLNode makeNestedDeepCell(int nestedLevel, int subLevel) {
        ...
        cell.addChild("#", "-");
        cell.addChild("span", "title", "sub level inside container/manifest",
                      ((subLevel == -1) ? "R" : Integer.toString(subLevel)));
    ...
    private HTMLNode makeTypeCell(Metadata md) {
        ...
        if (md.isArchiveInternalRedirect() || md.isArchiveMetadataRedirect()
            ...
            cell.addChild(
                "span", "title",
                "Metadata are in container, but points to external data (usually split files)",
                "[m]");
        }
    ...
    private HTMLNode makeTargetCell(Metadata md, String uri, HTMLNode htmlNode) {
        ...
        if ((md != null) && md.isSplitfile()) {
            ...
            cell.addChild("a", "href",
                          ...
                          + URLEncoder.encode(uri, false), "show split");
            cell.addChild("#", "]\u00a0");
            isEmpty = false;
        }
