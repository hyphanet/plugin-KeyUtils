...
//      }
    ...
    private void makeMainPage(ToadletContext ctx, List<String> errors)
            throws ToadletContextClosedException, IOException {
        makeMainPage(ctx, errors, null, 0);
    }

    private void makeMainPage(ToadletContext ctx, List<String> errors, String key, int level)
            throws ToadletContextClosedException, IOException {
        PageNode   page        =
            pluginContext.pageMaker.getPageNode(i18n("SplitExplorer.PageTitle"), ctx);
        ...
        HTMLNode uriBox = createUriBox(pluginContext,
                              ((furi == null) ? null : furi.toString(false, false)), level);
    ...
    private void makeSplitPage(ToadletContext ctx, List<String> errors, FreenetURI furi, int level)
            throws ToadletContextClosedException, IOException {
        PageNode   page        =
            pluginContext.pageMaker.getPageNode(i18n("SplitExplorer.PageTitle"), ctx);
        HTMLNode   outer       = page.outer;
    ...
    private HTMLNode createSplitBoxV0(PluginContext pCtx, Metadata md, String uri) {
        ...
        case Metadata.SPLITFILE_NONREDUNDANT :
            infoContent.addChild("#", "Non redundant");

            break;

        case Metadata.SPLITFILE_ONION_STANDARD :
            infoContent.addChild("#", "FEC Onion standard");

            break;

        default :
            infoContent.addChild("#", "<unknown>");
        }
        ...
        if (type == Metadata.SPLITFILE_ONION_STANDARD) {
            ...
            if ((params == null) || (params.length < 8)) {
                ...
                infoContent.addChild("#",
                                     "Segment count: "
                                     + ((sfDB / dataBlocksPerSegment)
                                        + ((sfDB % dataBlocksPerSegment == 0) ? 0 : 1)));
            }

            browseContent.addChild(createSegmentedBoxV0(pCtx, "data", md.getSplitfileDataKeys(),
                    dataBlocksPerSegment));
            browseContent.addChild(createSegmentedBoxV0(pCtx, "check", md.getSplitfileCheckKeys(),
    ...
    private HTMLNode createSegmentedBoxV0(PluginContext pCtx, String title, ClientCHK[] keys,
            int blockspersegment) {
        InfoboxNode box = pCtx.pageMaker.getInfobox("Split file " + title + " blocks: "
                              + keys.length);
        HTMLNode browseBox      = box.outer;
        HTMLNode browseContent  = box.content;
    ...
    private HTMLNode createSplitBoxV1(PluginContext pCtx, Metadata md, String uri) {
        ...
        if ( !md.isSplitfile()) {
            infoContent.addChild("#",
                                 "Error: Not a splitfile manifest?!@#?. may a bug, please report.");
        ...
        infoContent.addChild("br");
        infoContent.addChild("#",
                             "Compatiblity: " + md.getTopCompatibilityMode().name() + " (min: "
                             + md.getMinCompatMode().name() + " max: " + md.getMaxCompatMode()
                             + ")");
        infoContent.addChild("br");
        ...
        if (md.hasTopData()) {
            ...
            infoContent.addChild("br");
            infoContent.addChild("#",
                                 "\u00a0\u00a0Decompressed Size: " + Long.toString(md.topSize)
                                 + " bytes.");
            infoContent.addChild("br");
            infoContent.addChild("#",
                                 "\u00a0\u00a0Blocks: " + Integer.toString(md.topBlocksRequired)
                                 + " required, " + Integer.toString(md.topBlocksTotal) + " total.");
            infoContent.addChild("br");
        }
        ...
        try {
            ...
            infoContent.addChild(
                "#", "Error: Internal failure while decoding data. Try again (refresh the page).");
    ...
    private HTMLNode createSegmentedBoxV1(PluginContext pCtx, int index,
            SplitFileSegmentKeys segment) {
        InfoboxNode box           = pCtx.pageMaker.getInfobox("Segment #" + index);
        HTMLNode    browseBox     = box.outer;
        HTMLNode    browseContent = box.content;

        if (segment.dataBlocks > 0) {
            InfoboxNode segmentInfo = pCtx.pageMaker.getInfobox("Data Blocks: "
                                          + segment.dataBlocks);
            HTMLNode segmentBox     = segmentInfo.outer;
            HTMLNode segmentContent = segmentInfo.content;
        ...
        if (segment.checkBlocks > 0) {
            InfoboxNode segmentInfo = pCtx.pageMaker.getInfobox("Check Blocks: "
                                          + segment.checkBlocks);
            HTMLNode segmentBox     = segmentInfo.outer;
            HTMLNode segmentContent = segmentInfo.content;
    ...
    private HTMLNode createUriBox(PluginContext pCtx, String uri, int level) {
        ...
        if (uri != null) {
            browseForm.addChild("input", new String[] { "type", "name", "size", "value" },
                                new String[] { "text",
                    Globals.PARAM_LEVEL, "2", Integer.toString(level) });
        } else {
            browseForm.addChild("input", new String[] { "type", "name", "size", "value" },
                                new String[] { "text",
                    Globals.PARAM_LEVEL, "2", "0" });
        }
    ...
    private Metadata splitGet(PluginRespirator pr, FreenetURI uri, boolean last)
        ...
        if (result == null) {
            throw new FetchException(FetchException.INVALID_METADATA,
                                     "URI does not point to a split file");
        }

        return result;
    }

    private Metadata splitGet(PluginRespirator pr, FreenetURI uri, int level)
            throws FetchException {
