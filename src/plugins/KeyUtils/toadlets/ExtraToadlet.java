...
public class ExtraToadlet extends WebInterfaceToadlet {
    ...
    public void handleMethodPOST(URI uri, HTTPRequest request, ToadletContext ctx)
        ...
        if (path.startsWith(PATH_DECOMPOSE)) {
            ...
            // make result box
            InfoboxNode box = pluginContext.pageMaker.getInfobox("infobox-warning",
                                  "Decompose an Freenet URI's EXTRA: "
        ...
        if (path.startsWith(PATH_COMPOSE_CHK)) {
            ...
            // make result box
            InfoboxNode box = pluginContext.pageMaker.getInfobox("infobox-warning",
                                  "Composed an Freenet URI's EXTRA (CHK): ");
            HTMLNode resultBox     = box.outer;
            HTMLNode resultContent = box.content;
        ...
        if (path.startsWith(PATH_COMPOSE_SSK)) {
            ...
            // make result box
            InfoboxNode box = pluginContext.pageMaker.getInfobox("infobox-warning",
                                  "Composed an Freenet URI's EXTRA (SSK): ");
            HTMLNode resultBox     = box.outer;
            HTMLNode resultContent = box.content;
    ...
    private void makeMainPage(ToadletContext ctx, List<String> errors, String key)
            throws ToadletContextClosedException, IOException {
    ...
    private HTMLNode createUriBox(PluginContext pCtx, String uri, List<String> errors) {
        ...
        HTMLNode browseForm = pCtx.pluginRespirator.addFormChild(browseContent,
                                  path() + PATH_DECOMPOSE, "uriForm");
    ...
    private HTMLNode createComposeCHKBox(PluginContext pCtx, List<String> errors) {
        ...
        HTMLNode    browseContent = box.content;
        HTMLNode    browseForm    = pCtx.pluginRespirator.addFormChild(browseContent,
                                        path() + PATH_COMPOSE_CHK, "chkExtraForm");
        HTMLNode tableRow;
        HTMLNode cell;
        ...
        cell = tableRow.addChild("td");
        cell.addChild("input", new String[] { "type", "name", "size" }, new String[] { "text",
                "crypto_algo", "7" });
        cell.addChild("#", "\u00a0");

        HTMLNode cryptoselect = new HTMLNode("select", new String[] { "name", "onchange" },
                                    new String[] { "crypto_selector",
                "document.forms[\"chkExtraForm\"].elements[\"crypto_algo\"].value = this.options[this.options.selectedIndex].value" });
        ...
        cell.addChild("#", "\u00a0");
        cell = tableRow.addChild("td");
        cell.addChild("input", new String[] { "type", "name", "value" }, new String[] { "checkbox",
                "control_doc", "ok" });
        ...
        HTMLNode compressselect = new HTMLNode("select", new String[] { "name", "onchange" },
                                      new String[] { "compress_selector",
                "document.forms[\"chkExtraForm\"].elements[\"compress_algo\"].value = this.options[this.options.selectedIndex].value" });
    ...
    private HTMLNode createComposeSSKBox(PluginContext pCtx, List<String> errors) {
        ...
        HTMLNode    browseContent = box.content;
        HTMLNode    browseForm    = pCtx.pluginRespirator.addFormChild(browseContent,
                                        path() + PATH_COMPOSE_SSK, "sskExtraForm");
        HTMLNode tableRow;
        HTMLNode cell;
        HTMLNode table = browseForm.addChild("table", "class", "column");
        ...
        HTMLNode ssktype = new HTMLNode("select", new String[] { "name", "onchange" },
                               new String[] { "ssktype_selector",
                "document.forms[\"sskExtraForm\"].elements[\"ssk_type\"].value = this.options[this.options.selectedIndex].value" });
        ...
        HTMLNode cryptoselect = new HTMLNode("select", new String[] { "name", "onchange" },
                                    new String[] { "crypto_selector",
                "document.forms[\"sskExtraForm\"].elements[\"crypto_algo\"].value = this.options[this.options.selectedIndex].value" });
        ...
        HTMLNode hashselect = new HTMLNode("select", new String[] { "name", "onchange" },
                                  new String[] { "hash_selector",
                "document.forms[\"sskExtraForm\"].elements[\"hash_algo\"].value = this.options[this.options.selectedIndex].value" });
