...
public class StaticToadlet extends InvisibleWebInterfaceToadlet {
    ...
    public StaticToadlet(PluginContext context, String pluginUri, String path, String rezPath,
                         String mime, PluginL10n intl) {
    ...
    public void handleMethodGET(URI uri, HTTPRequest request, ToadletContext ctx)
        ...
        if (logDEBUG) {
            Logger.debug(this, "Requested ressource: " + path + " (" + uri + ')');
        }
        ...
        if (logDEBUG) {
            Logger.debug(this, "Resolved ressource path: " + fn);
        }
        ...
        if (fileInputStream == null) {
            if (logDEBUG) {
                Logger.debug(this, "Ressource not found.");
            }
        ...
        String mime  = (mimeType != null)
                       ? (mimeType) : (DefaultMIMETypes.guessMIMEType(path, false));
     ...
     * We usually load our resources from the JAR, or possibly from a file in some setups, so we
     * check the modification time of the JAR for resources in a jar and the mtime for files.
     */
