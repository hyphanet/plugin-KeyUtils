 ...
 *
 */
public class KeyUtilsPlugin
        implements FredPlugin, FredPluginL10n, FredPluginFCP, FredPluginThreadless,
                   FredPluginVersioned, FredPluginRealVersioned, FredPluginConfigurable,
                   FredPluginBaseL10n {
    ...
    private static final String PLUGIN_CATEGORY =
        "ConfigToadlet.plugins.KeyUtils.KeyUtilsPlugin.label";
    ...
    @Override
    public void handle(PluginReplySender replysender, SimpleFieldSet params, Bucket data,
                       int accesstype) {
        try {
    ...
    public void runPlugin(PluginRespirator pr) {
        ...
        webInterface.addNavigationCategory(PLUGIN_URI + "/", PLUGIN_CATEGORY,
                "Plugin.Category.tooltip", this);
        fcpHandler = new FCPHandler(pluginContext);

        // Visible pages
        KeyExplorerToadlet keyToadlet = new KeyExplorerToadlet(pluginContext, intl);

        webInterface.registerVisible(keyToadlet, PLUGIN_CATEGORY, "Menu.KeyExplorer.title",
                                     "Menu.KeyExplorer.tooltip");

        SiteExplorerToadlet siteToadlet = new SiteExplorerToadlet(pluginContext, intl);

        webInterface.registerVisible(siteToadlet, PLUGIN_CATEGORY, "Menu.SiteExplorer.title",
                                     "Menu.SiteExplorer.tooltip");

        SplitExplorerToadlet splitToadlet = new SplitExplorerToadlet(pluginContext, intl);

        webInterface.registerVisible(splitToadlet, PLUGIN_CATEGORY, "Menu.SplitExplorer.title",
                                     "Menu.SplitExplorer.tooltip");

        ExtraToadlet extraToadlet = new ExtraToadlet(pluginContext, intl);

        webInterface.registerVisible(extraToadlet, PLUGIN_CATEGORY, "Menu.ExtraCalculator.title",
                                     "Menu.ExtraCalculator.tooltip");

        FBlobToadlet fblobToadlet = new FBlobToadlet(pluginContext, intl);

        webInterface.registerVisible(fblobToadlet, PLUGIN_CATEGORY, "Menu.FBlobViewer.title",
                                     "Menu.FBlobViewer.tooltip");

        KeyConverterToadlet keyConverterToadlet = new KeyConverterToadlet(pluginContext, intl);

        webInterface.registerVisible(keyConverterToadlet, PLUGIN_CATEGORY,
                                     "Menu.KeyConverter.title", "Menu.KeyConverter.tooltip");

        AboutToadlet aboutToadlet = new AboutToadlet(pluginContext, intl);

        webInterface.registerVisible(aboutToadlet, PLUGIN_CATEGORY, "Menu.About.title",
                                     "Menu.About.tooltip");
        ...
        StaticToadlet cssToadlet = new StaticToadlet(pluginContext, KeyUtilsPlugin.PLUGIN_URI,
                                       "css", "/data/css/", "text/css", intl);

        webInterface.registerInvisible(cssToadlet);

        StaticToadlet picToadlet = new StaticToadlet(pluginContext, KeyUtilsPlugin.PLUGIN_URI,
                                       "images", "/data/images/", "image/png", intl);
