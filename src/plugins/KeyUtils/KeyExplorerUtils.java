...
public class KeyExplorerUtils {
    ...
    private static class SnoopGetter implements SnoopBucket {
        ...
        @Override
        public boolean snoopBucket(Bucket data, boolean isMetadata, ObjectContainer container,
    ...
    public static Metadata simpleManifestGet(PluginRespirator pr, FreenetURI uri)
            throws MetadataParseException, FetchException, IOException {
    ...
    public static GetResult simpleGet(PluginRespirator pr, FreenetURI uri) throws FetchException {
        ...
        ClientGetter get     = new ClientGetter(fw, uri, context,
                                   ...
                                   (RequestClient) pr.getHLSimpleClient(), null, null, null);
    ...
    public static FetchResult splitGet(PluginRespirator pr, Metadata metadata)
            throws FetchException, MetadataParseException, KeyListenerConstructionException {
        ...
        GetCompletionCallback cb  = new GetCompletionCallback() {
            ...
            public void onBlockSetFinished(ClientGetState state, ObjectContainer container,
                    ClientContext context) {}
            @Override
            public void onExpectedMIME(ClientMetadata metadata, ObjectContainer container,
                                       ClientContext context) {}
            @Override
            ...
            @Override
            public void onExpectedTopSize(long size, long compressed, int blocksReq,
                                          int blocksTotal, ObjectContainer container,
                                          ClientContext context) {}
            @Override
            ...
            public void onSplitfileCompatibilityMode(CompatibilityMode min, CompatibilityMode max,
                    ...
                    boolean definitiveAnyway, ObjectContainer container, ClientContext context) {}
            @Override
            public void onSuccess(StreamGenerator streamGenerator, ClientMetadata clientMetadata,
                ...
                try {
                    ...
                    if (decompressors != null) {
                        ...
                        decompressorManager = new DecompressorThreadManager(dataInput,
                                decompressors, maxLen);
                        dataInput = decompressorManager.execute();
                    }

                    output = finalResult.getOutputStream();
                    worker = new ClientGetWorkerThread(dataInput, output, null, null, null, false,
                            ctx.charset, ctx.prefetchHook, ctx.tagReplacer, null);
                    worker.start();
                    ...
                    if (logMINOR) {
                        Logger.minor(this, "Size of written data: " + result.asBucket().size());
                    }
                    ...
                    onFailure(new FetchException(FetchException.INTERNAL_ERROR, e), state,
                              container, context);
                    ...
                    onFailure(new FetchException(e.getFetchErrorCode(), e), state, container,
                              context);
                    ...
                    // Impossible
                    Logger.error(this,
                                 "URISyntaxException converting a FreenetURI to a URI!: " + e, e);
                    onFailure(new FetchException(FetchException.INTERNAL_ERROR, e),
                              state /* Not really the state's fault */, container, context);
                    ...
                    onFailure(new FetchException(FetchException.TOO_BIG, e), state, container,
                              context);
                    ...
                    Logger.error(this, "Caught " + e, e);
                    onFailure(new FetchException(FetchException.BUCKET_ERROR, e), state, container,
                              context);
                    ...
                    onFailure(new FetchException(FetchException.INTERNAL_ERROR, t), state,
                              container, context);
        ...
        SplitFileFetcher sf = new SplitFileFetcher(metadata, cb, vsg, ctx, deleteFetchContext,
                                  true, decompressors, clientMetadata, actx, recursionLevel, token,
    ...
    public static Metadata splitManifestGet(PluginRespirator pr, Metadata metadata)
            throws MetadataParseException, IOException, FetchException,
    ...
    public static Metadata zipManifestGet(PluginRespirator pr, FreenetURI uri)
    ...
    public static Metadata tarManifestGet(PluginRespirator pr, Metadata md, String metaName)
            throws FetchException, MetadataParseException, IOException {
        FetchResult fr;
    ...
    public static Metadata tarManifestGet(PluginRespirator pr, FreenetURI uri, String metaName)
            throws FetchException, MetadataParseException, IOException {
        HighLevelSimpleClient hlsc = pr.getHLSimpleClient();
        FetchContext          fctx = hlsc.getFetchContext();
    ...
    public static Metadata internalTarManifestGet(Bucket data, String metaName)
            throws IOException, MetadataParseException, FetchException {
    ...
    public static HashMap<String, Object> parseMetadata(Metadata oldMetadata, FreenetURI oldUri)
            throws MalformedURLException {
        return parseMetadata(oldMetadata.getDocuments(), oldUri, "");
    }

    private static HashMap<String, Object> parseMetadata(HashMap<String, Metadata> oldMetadata,
            FreenetURI oldUri, String prefix)
            throws MalformedURLException {
        ...
        for (Entry<String, Metadata> entry : oldMetadata.entrySet()) {
            ...
            if (md.isArchiveInternalRedirect()) {
                ...
                newMetadata.put(name,
                                parseMetadata(md.getDocuments(), oldUri, prefix + name + "/"));
            }
        }
    ...
    private byte[] doDownload(PluginRespirator pluginRespirator, List<String> errors, String key) {
        ...
        try {
            ...
            if (getresult.isMetaData()) {
                return unrollMetadata(pluginRespirator, errors,
    ...
    public static byte[] unrollMetadata(PluginRespirator pluginRespirator, List<String> errors,
            Metadata md)
            throws MalformedURLException, IOException, FetchException, MetadataParseException,
                   KeyListenerConstructionException {
        if ( !md.isSplitfile()) {
