...
public class FCPHandler extends AbstractFCPHandler {
     ...
     */
    @Override
    public void handle(PluginReplySender replysender, String command, String identifier,
                       SimpleFieldSet params, Bucket data, int accesstype)
            throws PluginNotFoundException {
        if (params == null) {
        ...
        if (data != null) {
            sendError(replysender, 0, identifier,
                      "Got a diatribe piece of writing. Data not allowed!");
        ...
        if ("Get".equals(command)) {
            ...
            try {
                FreenetURI furi = URISanitizer.sanitizeURI(uri, URISanitizer.Options.NOMETASTRINGS,
                                      URISanitizer.Options.SSKFORUSK);
                GetResult getResult = KeyExplorerUtils.simpleGet(pluginContext.pluginRespirator,
                                          furi);
